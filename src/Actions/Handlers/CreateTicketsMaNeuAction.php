...
use Hwkdo\IntranetAppWorkflows\Support\MaNeuTicketPlanner;
use Hwkdo\IntranetAppWorkflows\Support\PhaseCGuard;
use Illuminate\Support\Facades\Mail;
use Throwable;

final class CreateTicketsMaNeuAction implements WorkflowActionInterface
{
    ...
    public function handle(ActionContext $context): ActionResult
    {
        ...
        $planner = new MaNeuTicketPlanner($this->bue);
        ...
        if ($dry = PhaseCGuard::assertNotDryRunOrMessage(
            actionLabel: 'Tickets erstellen ('.count($tickets).')',
        ...
        if ($errors !== [] && $messages === []) {
            return ActionResult::failed(
                message: 'Keine Ticket-Mails konnten geplant werden',
                errors: $errors,
            );
        }
        ...
        if ($errors !== []) {
            return ActionResult::partial(
                message: 'Ticket-Mails teilweise geplant',
                messages: $messages,
                errors: $errors,
        ...
        return ActionResult::succeeded(
            message: count($messages).' Ticket-Mails geplant',
            output: ['tickets_queued' => count($messages)],
            messages: $messages,
        );
    }
