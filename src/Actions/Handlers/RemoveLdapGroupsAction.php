...
final class RemoveLdapGroupsAction implements WorkflowActionInterface
{
    ...
    {
        return 'ma_umsetzung.remove_ldap_groups';
    }
    ...
    public function handle(ActionContext $context): ActionResult
    {
        ...
        $groups = $this->normalizeGroups($context->payloadValue('remove_ldap_groups'));
        if ($groups === []) {
            return ActionResult::succeeded('Keine LDAP-Gruppen zu entfernen');
        }

        if ($dry = PhaseBGuard::assertNotDryRunOrMessage(
            'LDAP-Gruppen entfernen für ['.$username.']: '.count($groups).' geplant'
        )) {
            return ActionResult::succeeded(
                message: $dry->message,
                output: array_merge($dry->output, ['ldap_groups_remove_planned' => $groups]),
        ...
        foreach ($groups as $group) {
            if ($this->ldap->removeUserFromGroup($username, $group)) {
                $ok[] = $group;
            } else {
                $errors[] = $group;
            }
        ...
        if ($errors !== [] && $ok === []) {
            return ActionResult::failed(
                message: 'Keine LDAP-Gruppe konnte entfernt werden',
                errors: $errors,
            );
        }
        ...
        if ($errors !== []) {
            return ActionResult::partial(
                ...
                output: ['ldap_groups_removed' => $ok, 'ldap_groups_remove_failed' => $errors],
            );
        }

        return ActionResult::succeeded(
            message: count($ok).' LDAP-Gruppe(n) entfernt',
            output: ['ldap_groups_removed' => $ok],
            messages: array_map(fn (string $g): string => "OK: {$g}", $ok),
        );
    }
